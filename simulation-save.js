...
let x=Math.random()
var nodesD = [{"id": 1, "weight": 0,"state":x<starter? "infected":"sane","credule": x<p,'neighbours':[]}]
var nodesWeighted = [1]
...
function update() {
    ...
    if (mode==3){
        ...
        for (let e in edgesD){
            s=edgesD[e].source.id-1
            t=edgesD[e].target.id-1
    ...
    nodes=nodesG.selectAll(".node")
        ...
        .style('fill',function(d){
            if (mode<=1) {//remplissage normal
                return fill(d.id)}
            if (mode==2) {//tout le monde en noir
                return 'black'
            }
            if (mode==3){//sceptiques en bleu
                return d.credule? 'red':'blue'
            }
            if (mode>3){//starters en noir
                if (d.state=="incube"){
                    nodesD[d.id - 1].state = "infected";
                    console.log(d.id,'infected')
                    infected.add(d.id)
                }
                result= d.credule? d.state=="infected"? 'black':'red' : 'blue'
                if (mode>4 && d.credule){
                    for (let n in d.neighbours){
                        nNode=nodesD[d.neighbours[n]]
                        if (nNode.state=="infected" && d.state=="sane") {
                            console.log(d.id,'infected by ',nNode.id)
                            nodesD[d.id - 1].state = "incube";
                        }
                    }
                    //d.infected si neighbour infected
                }
                return result
    ...
    edges=edgesG.selectAll(".edge")
        ...
        .enter()
        .append("line")
        .style("stroke-width",d=>mode>3? d.source.state=="infected"?  d.target.state=="infected"? 10 : 2 : 2 :2)
        .attr("class", "edge")
